...
Conduct a deep design interview about a plan, idea, or project until a shared understanding is reached. Walk down each branch of the design tree, resolving dependencies between decisions one-by-one.
...
**Core rule:** If a question can be answered by exploring available files or context, do so instead of asking. Only ask what cannot be discovered from context.

## Topic

User's topic, plan, or project: `$ARGUMENTS`

If no topic provided, ask: "What plan or idea would you like me to grill you on?"
...
5. **Use what you know** — before asking, check available files, git history, existing docs, and code structure. State what you found and ask only about gaps.
...
8. **Build incrementally** — maintain a running "decisions made" block so the user can see what's been resolved.
